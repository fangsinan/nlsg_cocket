...
class Status
{
    ...
    static function Error($statusCode = 200, $msg = '', $result = []){

        if(!$msg){
            $msg=self::$phrases[$statusCode]?self::$phrases[$statusCode]:$msg;
        }
        return ['statusCode'=>$statusCode,'msg'=>$msg,'result'=>$result];
    }

    static function Success( $msg = '', $result = []){
    ...
    static function isSuccess($res){
        ...
        if($res['statusCode']!==self::CODE_OK){
            return false;
        }else{
            return true;
        }
